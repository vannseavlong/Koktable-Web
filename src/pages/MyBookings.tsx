...
function BookingCard({ booking, past }: { booking: Booking; past?: boolean }) {
  ...
  const { data: r, isLoading } = useRestaurantById(
    booking.restaurantId || undefined,
  )
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              {past ? (
                ...
                <>
                  <Link
                    to={ROUTES.restaurantDetail(r.id)}
                  ...
                  <span className="text-border">|</span>
                  <Link
                    to={ROUTES.manageBooking(booking.ref)}
                    className="text-xs text-terra font-medium hover:underline"
                  >
                    {t("myBookings.manage")}
                  </Link>
    ...
    <div className="min-h-screen bg-cream">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link
            to={ROUTES.restaurants}
            className="px-4 py-2 bg-terra text-white rounded-lg text-sm font-medium hover:bg-terra-dark transition-all"
          >
        ...
        {isError && (
          <p className="text-sm text-destructive text-center py-24">
          ...
          (upcoming.length === 0 ? (
            <div className="text-center py-24">
              ...
              <h3 className="font-display text-xl font-semibold text-ink mb-2">
                {t("myBookings.noUpcomingTitle")}
              </h3>
              <p className="text-ink-muted text-sm mb-6">
                {t("myBookings.noUpcomingSubtitle")}
              </p>
              <Link
                to={ROUTES.home}
